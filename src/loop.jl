...
function loopfunppSmooth(p0, ps, k, m, T,slop=5)
    if m >= 0
        if k > ps
            if p0 >= Epi(k + ps, m) + Epi(k, m)
                qm =
                ...
                qp = qm + ps
                return star1fun(qp, qm, ps, m, T)
            elseif 1.7*Epi(k, m)<p0 < Epi(k + ps, m) + Epi(k, m)
                qm = sqrt((p0 - Epi(k, m))^2 - m) - ps
                qp = sqrt((p0 - Epi(k, m))^2 - m)
                return 0.5*(star2fun(qp, qm, ps, k, m, T) + star3fun(qp, ps, k, m, T))*(tanh(slop*(p0-2 * Epi(k, m)))+1)
            elseif p0<=1.7*Epi(k, m)
                return 0.0
            end
        elseif ps / 2 < k <= ps
            if p0 >= Epi(k + ps, m) + Epi(k, m)
            ...
            elseif Epi(ps, m) + Epi(k, m) <= p0 < Epi(k + ps, m) + Epi(k, m)
                ...
                qp = sqrt((p0 - Epi(k, m))^2 - m)
                return star2fun(qp, qm, ps, k, m, T) + star3fun(qp, ps, k, m, T)
            elseif Epi(k, m)<p0 < Epi(ps, m) + Epi(k, m)
                qm = ps - sqrt((p0 - Epi(k, m))^2 - m)
                qp = sqrt((p0 - Epi(k, m))^2 - m)
                return 0.5*(star2fun(qp, qm, ps, k, m, T) + star3fun(qp, ps, k, m, T))*(tanh(slop*(p0-2 * Epi(k, m)))+1)
            elseif p0<=Epi(k, m)
