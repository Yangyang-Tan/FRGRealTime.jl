...
@time tsolTwoPointparallelTc = @showprogress pmap(p0grid_Tc) do p0
    tchanelSolveTwoPointParallel5(
        p0,
        Tcminmax[2] - twopointReminmax[1],
        lpakrang,
...
writedlm(joinpath(path,"N=1/redataN=1.dat"),redata_twopoint)
writedlm(joinpath(path,"N=1/imdataN=1.dat"),imdata_twopoint)
writedlm(joinpath(path,"N=1/p0dataN=1.dat"),p0grid_Tc)
